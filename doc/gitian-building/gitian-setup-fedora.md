...
The build script we'll be using is contained in the Cryptcore github repository ([contrib/gitian-build.py](https://github.com/cryptcore-project/cryptcore/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the Cryptcore repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/Cryptcore-Project/Cryptcore/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, cryptcore-detached-sigs, and cryptcore GitHub repos.
