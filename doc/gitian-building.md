...
*Setup instructions for a gitian build of Cryptcore using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Cryptcore
...
to the Cryptcore GitHub release page.
...
- [Building Cryptcore](#building-cryptcore-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/cryptcore-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/cryptcore-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/cryptcore-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/cryptcore-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/cryptcore-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/cryptcore-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/cryptcore-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@cryptcore.trade) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/cryptcore-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/cryptcore-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/cryptcore-osx-*-build.assert
```
