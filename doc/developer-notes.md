...
- [Development guidelines](#development-guidelines)
    - [General Cryptcore](#general-cryptcore-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Cryptcore uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play CORRs (tCORR)" on the test network, if you
...
Cryptcore is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/cryptcore-project/cryptcore/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_cryptcore
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_cryptcore --log_level=test_suite
$ valgrind -v --leak-check=full src/cryptcored -printtoconsole
```
...
# A coverage report will now be accessible at `./test_cryptcore.coverage/index.html`.
```
...
- BitcoinMiner : Generates CORRs (if wallet is enabled).
...
However, in open source software such as Cryptcore, where everyone uses
...
pay attention to for reviewers of Cryptcore code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Cryptcore uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Cryptcore so that they can be integrated
...
file descriptors it will cause problems with Cryptcore's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Cryptcore-Project/Cryptcore.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `cryptcore-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `cryptcore-cli` and the GUI debug console use this table to determine how to
