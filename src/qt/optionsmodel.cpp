...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::CORR);
    ...
    if (settings.contains("nAnonymizeCryptcoreAmount") || reset)
        SoftSetArg("-anonymizecryptcoreamount", settings.value("nAnonymizeCryptcoreAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cryptcore.cpp during shotdown to also remove the window positions
