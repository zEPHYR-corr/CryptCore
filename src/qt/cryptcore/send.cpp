...
#include "qt/cryptcore/forms/ui_send.h"
#include "qt/cryptcore/addnewcontactdialog.h"
#include "qt/cryptcore/qtutils.h"
#include "qt/cryptcore/sendchangeaddressdialog.h"
#include "qt/cryptcore/optionbutton.h"
#include "qt/cryptcore/sendconfirmdialog.h"
#include "qt/cryptcore/myaddressrow.h"
#include "qt/cryptcore/guitransactionsutils.h"
...
SendWidget::SendWidget(CryptcoreGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-corr");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination corrAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(corrAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(corrAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
