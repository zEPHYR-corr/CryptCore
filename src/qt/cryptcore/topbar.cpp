...
#include "qt/cryptcore/forms/ui_topbar.h"
#include "qt/cryptcore/lockunlock.h"
#include "qt/cryptcore/qtutils.h"
#include "qt/cryptcore/receivedialog.h"
...
TopBar::TopBar(CryptcoreGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopCORR}, "amount-small-topbar");
    setCssProperty({ui->labelAmountCORR}, "amount-topbar");
    setCssProperty({ui->labelPendingCORR, ui->labelImmatureCORR}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("Cryptcore")
    updateDisplayUnit();
                            ...
                            const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    // CORR Total
    QString totalCORR = GUIUtil::formatBalance(balance, nDisplayUnit);

    // CORR
    // Top
    ui->labelAmountTopCORR->setText(totalCORR);
    // Expanded
    ui->labelAmountCORR->setText(totalCORR);
    ui->labelPendingCORR->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmatureCORR->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
}
