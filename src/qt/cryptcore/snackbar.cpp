...
#include "qt/cryptcore/forms/ui_snackbar.h"
#include "qt/cryptcore/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(CryptcoreGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &CryptcoreGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
