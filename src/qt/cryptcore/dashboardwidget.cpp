...
#include "qt/cryptcore/dashboardwidget.h"
#include "qt/cryptcore/forms/ui_dashboardwidget.h"
#include "qt/cryptcore/sendconfirmdialog.h"
#include "qt/cryptcore/txrow.h"
...
DashboardWidget::DashboardWidget(CryptcoreGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareCORR, "square-chart-corr");
    setCssProperty(ui->labelCORR, "text-chart-corr");
    ...
    setCssProperty(ui->labelAmountCORR, "text-stake-corr-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &CryptcoreGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &CryptcoreGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &CryptcoreGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            corr = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalCORR += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesCORR.append(corr);

        int max = std::max(corr, corr);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesCORR);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalCORR > 0) {
        setCssProperty(ui->labelAmountCORR, "text-stake-corr");
    } else {
        setCssProperty(ui->labelAmountCORR, "text-stake-corr-disable");
    }
    forceUpdateStyle({ui->labelAmountCORR});
    ui->labelAmountCORR->setText(GUIUtil::formatBalance(chartData->totalCORR, nDisplayUnit));
