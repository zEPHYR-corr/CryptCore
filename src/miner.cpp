...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("Cryptcore CoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("Cryptcore CoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("Cryptcore CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("cryptcore-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running Cryptcore CoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("Cryptcore CoinMiner exception");
    } catch (...) {
        LogPrintf("Cryptcore CoinMiner exception");
    }

    LogPrintf("Cryptcore CoinMiner exiting\n");
}
